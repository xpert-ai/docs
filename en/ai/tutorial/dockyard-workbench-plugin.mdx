...
The example has API-to-model explanation evidence. Current-version runtime screenshots and the complete UI workflow for manual adoption, save/restore, and failure recovery still need final acceptance. Do not label API receipts or upstream screenshots as browser acceptance for this plugin.
