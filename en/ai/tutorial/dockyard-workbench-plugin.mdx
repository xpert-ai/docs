...
The tutorial focuses on responsibilities, implementation steps, and validation. The complete example is in [community/apps/dockyard](https://github.com/xpert-ai/xpert-plugins/tree/main/community/apps/dockyard) in the official repository, including plugin source, tests, screenshots, and supporting development documents.

<Warning>
This tutorial describes Dockyard plugin 0.3.0. Context-menu references require the host command `assistant.composer.append_references`. Confirm that your deployment includes this capability before reproducing the reference flow. Installing the plugin alone does not establish support on an unmodified Xpert main checkout. The command is not presented here as a stable capability available in every version.
</Warning>
...
To run the example first, clone the official repository into a separate directory. The plugin is under `community/apps/dockyard`:
...
Use your own feature branch when developing a plugin, and check for differences between your source version and the 0.3.0 example described here. Record the plugin, host, and upstream UI commit SHAs instead of recording only “latest.” This example uses Node 22, pnpm 8.15.8 for the plugin, and SDK/contracts 3.18.4. Select the host package manager version from its own package.json.
...
The complete message also needs a protocol channel, version, instance ID, and request ID. Validate the message source, instance, and response type, and handle timeouts and disposal so a response from another instance cannot complete this request. See `src/lib/remote/bridge.ts` in the example.
...
- [Dockyard plugin source](https://github.com/xpert-ai/xpert-plugins/tree/main/community/apps/dockyard)
- [Workbench remote components](/en/ai/plugin/plugin-sdk/remote-component)
- [Plugin publishing and usage](/en/ai/plugin/install)
- [Example and six development documents](https://github.com/xpert-ai/xpert-plugins/tree/main/community/apps/dockyard/docs/interview)
