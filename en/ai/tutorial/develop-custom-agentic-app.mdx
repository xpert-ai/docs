...
- [Workbench and ChatKit](../../data/features/agent-execution/workbench-and-chatkit), [ChatKit](../chatkit), and [Workbench Remote Components](../plugin/plugin-sdk/remote-component)
...
- `meta.level`: `system`, `tenant`, or `organization`. A BOM-style app with server modules that serves one customer tenant should use `tenant`.
- `meta.artifactNamespace`: the stable artifact namespace for a process-level plugin, used to derive table names, routes, and registry keys.
...
const PLUGIN_ARTIFACT_NAMESPACE = 'contract_review' as const
...
- A tenant-level plugin's `package.json` must declare the same `level` and `artifactNamespace`; restart the API after install, update, or uninstall.
...
A BOM-style plugin registers a fixed view, such as `BOM Review`, through a view provider. For iframe-based plugin UI, see [Workbench Remote Components](../plugin/plugin-sdk/remote-component). Its manifest declares:
...
Then register the independent plugin package into the target tenant through the plugin install API or the platform deployment command:

```bash
pnpm plugin:deploy:local \
  --plugin-dir /abs/path/to/xpert-plugins/acme/contract-review \
  --scope tenant \
  --tenant-id <your-tenant-id>
```

After deploying a tenant-level plugin, restart the API as requested, then verify that it works in the target tenant and remains invisible to other tenants. After code changes, rebuild and deploy the package again, then restart the API to activate the new version. This also keeps the host application and plugin repository from sharing duplicated runtime dependencies accidentally.
