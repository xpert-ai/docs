...
XpertAI automatically maintains the file name, logical knowledge base folder path, file extension, MIME type, document category, source type, creation time, and update time. A folder path represents the hierarchy inside the knowledge base and never exposes a server storage path.

Renaming or moving a folder updates the logical paths of its descendant documents. Renaming a file, moving a folder, or updating metadata changes retrieval attributes without recomputing embeddings.
