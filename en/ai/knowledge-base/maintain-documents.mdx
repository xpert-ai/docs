...
For PDF files, a knowledge pipeline can use Baidu PaddleOCR‑VL, Unlimited‑OCR, or MinerU document converter plugins. PaddleOCR‑VL preserves paged layouts, tables, images, and coordinates. MinerU supports official Precise Parsing and self-hosted services and can preserve Markdown, structured JSON, model output, and images. After PaddleOCR‑VL processing, select **Open analysis preview** from the document list to synchronize scrolling and analysis-block selection between the original PDF and parsed result.

The **Analysis preview** in document details represents pre-split layout blocks, while **Chunks** represents final index chunks produced by the chunker. See [PDF Processing and Linked Analysis Preview](/en/ai/knowledge-base/pdf-processing-and-preview) for configuration, support boundaries, and reprocessing behavior.
...
- **Playwright Operation Steps**:  
  Enter the full URL of the target web page in the input box (e.g., https://xpertai.cn/), and click "Load" to view the web scraping results below. Click "Preview" to see the web content. Click "Next" to configure it the same way as local documents, then save and process.
...
## Managing system attributes and business metadata

In addition to document content, XpertAI maintains structured attributes for precise retrieval.

### Automatically maintained system attributes

XpertAI automatically maintains the file name, logical knowledge base folder path, file extension, MIME type, document category, source type, creation time, and update time. A folder path represents the hierarchy inside the knowledge base and never exposes a server storage path. For example, **Water Resources > East China** is stored as `Water Resources/East China`, without leading or trailing slashes; the root folder is stored as an empty value.

Renaming or moving a folder updates the logical paths of its descendant documents. Renaming a file, moving a folder, or updating metadata changes retrieval attributes without recomputing embeddings. The separate `filePath` storage identifier can remain unchanged and must not be used as `document.folderPath`.

### Defining the metadata schema

Administrators can register business fields in the knowledge base metadata schema. Each field contains:

| Setting | Description |
| --- | --- |
| Key | Stable identifier such as `domain`, `effectiveYear`, or `region`. |
| Type | String, enum, number, datetime, boolean, string array, number array, or object. |
| Scope | `document` for a document-level field or `chunk` for a chunk-level field. |
| Enum values | Allowed values for an enum field. |
| Label and description | Guidance that helps administrators and agents interpret the field correctly. |

The server validates actual values against the schema. For example, a year should be stored as a number, an effective flag as a boolean, and a timestamp as an ISO-8601 datetime. Do not convert every business value to a string.

### Maintaining metadata

Business metadata can be supplied during document upload or updated from document details, chunk editing, and bulk-maintenance operations:

- document-scoped fields apply to the entire document and all of its chunks;
- chunk-scoped fields apply only to the corresponding chunk;
- metadata updates become available to PGVector or Milvus filtering immediately;
- if validation reports inconsistent historical values, correct the data before using that field in a production filter.

See [Intelligent Retrieval Filtering](/en/ai/knowledge-base/intelligent-filtering) to use these attributes in retrieval boundaries.

## Managing Chunks

When a document converter provides a source map, final chunks retain their source-page range, contributing layout block IDs, and related assets. For PaddleOCR‑VL PDFs, this provenance comes from the Markdown source map produced during analysis and supports later citations and diagnostics.
