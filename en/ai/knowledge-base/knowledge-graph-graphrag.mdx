...
2. Progressively scan a bounded set of candidates and keep only active entities with eligible evidence inside the effective Knowledge Filter scope.
3. Expand only active relationships that have eligible evidence inside the same scope, up to the configured Neighbor Hops and graph safety limits.
4. Resolve eligible entity and relationship mentions back to enabled source chunks that satisfy the same document-and-chunk filter expression.
5. Calculate graph relevance using entity similarity and evidence confidence, then return the highest-ranked source chunks with matched entity and relationship metadata.
...
### Knowledge Filters in Graph and Hybrid

Knowledge Filter V2 supports Vector, Graph, and Hybrid retrieval. The effective boundary is always the tenant, organization, knowledge base, enabled-content state, fixed filter, caller filter, and valid Agent dynamic filter combined with `AND`.

- **Graph** applies the boundary to seed eligibility, relationship expansion, and final evidence chunks.
- **Hybrid** applies the same boundary independently to both Vector and Graph before deduplication and score fusion.
- A missing or invalid fixed-filter variable stops retrieval before graph seed search; it is never ignored.
- An invalid Agent dynamic filter is discarded as one unit, and Graph or Hybrid continues with the fixed and caller boundaries while recording `invalid_dynamic_filter`.
- A valid dynamic filter with zero hits is not automatically relaxed. The Agent decides whether to call again without the dynamic condition.
- Graph errors fail Graph mode explicitly. Hybrid may fall back only to the already-filtered Vector branch and records the graph fallback reason.

The graph path uses parameterized PostgreSQL joins across graph mentions, documents, and chunks. A mixed document/chunk `AND` or `OR` expression is therefore evaluated against the same evidence row, and disabled documents or chunks cannot re-enter through graph expansion.
...
- combine the discoveries in the final Retriever query and optional dynamic filter, regardless of whether that binding uses Vector, Graph, or Hybrid.
...
When a fixed filter is active, global entity descriptions, summaries, aliases, and aggregate counts are not exposed because they may have been aggregated from documents outside the binding. Missing fixed variables fail closed before graph exploration starts. If Agent automatic filtering is enabled, the graph explorer may also add a literal-only dynamic filter; invalid dynamic conditions fall back to the fixed boundary and are reported. Graph quotes are exploration clues; the Agent is instructed to use the Retriever for final chunks and citations.
...
Inspect returned chunk content, graph score, matched entities, matched relationships, citations, and latency. Filter diagnostics also show the global entity candidates scanned, scan rounds and cap, eligible seed entities, eligible relationships and mentions, candidate chunks, per-branch hit counts, graph-stage latency, truncation, and any Hybrid graph fallback. Also verify graph results in the visual workspace before increasing Entity Top K or Neighbor Hops. See [Recall Test](/en/ai/knowledge-base/recall-test).
...
- Entity vectors remain knowledge-base-wide. Filtered Graph retrieval progressively scans global candidates up to a hard cap and then gates them by eligible evidence. Returned entities, relationships, and chunks remain strictly in scope, but an in-scope seed ranked beyond the scan cap can be missed. A truncation flag is available in diagnostics; raise Entity Top K carefully or use Hybrid when this affects recall.
- Use the Agent graph explorer as a boundary-aware planning step when the best entity names or relationships are not yet known.
