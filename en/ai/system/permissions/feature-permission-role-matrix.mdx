...
Application initialization, private workspace membership and group authorization for a published Assistant are independent layers. See [Assistant and Application Authorization](/en/ai/system/permissions/assistant-application-authorization) for employee onboarding and troubleshooting.
...
| Application initialization | Initialize an application in Explore | Inherits the application entry | `SUPER_ADMIN`, `ADMIN` or `TRIAL`, with organization and model requirements satisfied | Creates a private workspace; does not automatically grant employees runtime access. |
| Published Assistant | Business user access | Inherits the Chat entry | Member of an authorized organization group, or eligible as creator, workspace owner or explicit workspace member | Users do not need `ORG_USERS_VIEW` or `ORG_USERS_EDIT`. |
