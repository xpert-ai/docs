...
应用初始化、私有工作空间成员和已发布 Assistant 的用户组授权是独立层级。员工接入步骤及排查说明见 [Assistant 与应用授权](/zh-Hans/ai/system/permissions/assistant-application-authorization)。
...
| 应用初始化 | Explore 应用初始化 | 继承应用入口 | `SUPER_ADMIN`、`ADMIN` 或 `TRIAL`，并满足组织和模型条件 | 新建私有工作空间；不自动分配员工运行授权。 |
| 已发布 Assistant | 业务用户使用 | 继承 Chat 入口 | 用户属于获授权组织用户组，或满足创建者、空间所有者、显式空间成员条件 | 使用者无需获得 `ORG_USERS_VIEW` 或 `ORG_USERS_EDIT`。 |
