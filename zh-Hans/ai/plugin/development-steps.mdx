...
如果插件需要在 Explore 作为应用展示，并初始化私有工作空间及已发布 Assistant，请同时阅读[应用初始化配置](/zh-Hans/ai/plugin/application-config)。员工使用权限需要通过 [Assistant 用户组授权](/zh-Hans/ai/system/permissions/assistant-application-authorization)单独分配。
...
* `organization`：默认选择。适合普通工具、集成、工作流和不注册进程级基础设施的业务插件。
* `tenant`：适合只属于一个客户 tenant，但需要注册 Nest Module、Controller、TypeORM Entity 或其他进程级 Provider 的行业应用。
* `system`：仅用于所有 tenant 共享的平台级单例能力。

Tenant-level 插件必须在运行时入口和包元数据中同时声明 level 与稳定的制品命名空间：
...
两个元数据面必须保持一致。`artifactNamespace` 只能使用小写字母、数字和下划线，并且发布后应保持稳定。数据库表建议使用 `plugin_<artifactNamespace>_<tableKey>`，Controller 路由、注册表 key、队列和缓存标识也应从同一个常量派生，避免不同插件之间发生全局命名冲突。

Tenant-level 与 system-level 都属于进程级插件。它们的安装、更新和卸载不会直接修改正在运行的 Nest 模块图，而是在重启 API 后激活或卸载。开发和验收时必须把“重启后验证”纳入测试步骤。
...
### 7.1 Webhook Controller 与 scoped credential

如果插件需要暴露给外部系统调用的 webhook endpoint，应使用 SDK 提供的 `PluginWebhookAuthGuard`，不要让用户把完整平台 API Key 放进 webhook URL。

宿主通过 `IntegrationPermissionService` 管理每个 Integration 独立的 webhook credential：

* `ensureWebhookCredential(id, { provider })` 创建或返回当前有效 credential，并只在这次调用中返回明文 token。
* `rotateWebhookCredential(id, { provider })` 轮换 token。
* `revokeWebhookCredential(id, { provider })` 吊销 token。
* Integration 只保存 `options.webhookCredential.tokenHash` 和元数据，不保存明文 token。

callback URL 中只放 opaque query secret：
...
当 webhook route 需要跳过全局登录鉴权时，使用宿主已有的 public-route decorator。不要把 `PluginWebhookAuthGuard` 加成全局 bootstrap guard，应只通过 `@UseGuards()` 挂到 webhook route 上。`@PluginWebhookAuth()` 中应始终声明 `provider`，避免一个 Integration provider 的 credential 被重放到另一个 provider route。
