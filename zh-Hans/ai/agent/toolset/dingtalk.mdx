...
- 推荐优先使用 **钉钉-Stream模式**；只有无法使用 Stream 模式时，再使用 **钉钉-HTTP模式**
- 群聊中仅处理 `@机器人` 消息
...
如果使用 Stream 模式，不需要公网回调地址和回调安全参数，只需要服务端能够访问钉钉 Stream 网关。
...
进入 **设置 -> 系统集成**，按部署方式创建钉钉集成：

- 推荐：创建 **钉钉-Stream模式** 集成。
- 备用：创建 **钉钉-HTTP模式** 集成。
...
如果使用 HTTP 模式，集成保存后可在系统集成“测试”中拿到回调地址：
