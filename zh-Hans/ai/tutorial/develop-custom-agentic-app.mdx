...
- [工作台与资源对话](../../data/features/agent-execution/workbench-and-chatkit)、[ChatKit](../chatkit) 与[工作台远程组件](../plugin/plugin-sdk/remote-component)
...
- `meta.level`：`system`、`tenant` 或 `organization`。BOM 这类拥有服务端模块但只服务一个客户租户的应用应使用 `tenant`。
- `meta.artifactNamespace`：进程级插件的稳定制品命名空间，用于派生表名、路由和注册表 key。
...
- Tenant-level 插件的 `package.json` 必须声明相同的 `level` 和 `artifactNamespace`；安装、更新或卸载后需要重启 API。
...
BOM 类插件会通过视图提供器注册一个固定视图，例如 `BOM 审核台`。如果需要 iframe 形式的插件界面，可参考[工作台远程组件](../plugin/plugin-sdk/remote-component)。它的清单声明了：
...
然后通过插件安装接口或项目提供的部署脚本，把独立插件包注册到目标 tenant：
...
Tenant-level 插件部署完成后应按平台提示重启 API，再验证目标 tenant 可用且其他 tenant 不可见。代码变更后，重新构建并部署插件，重启 API 激活新版本。这样可以避免宿主应用和插件仓库之间出现重复的运行时依赖。
