...
教程重点是接入过程中的职责划分、实现步骤和验证方法。完整示例位于官方仓库的 [community/apps/dockyard](https://github.com/xpert-ai/xpert-plugins/tree/main/community/apps/dockyard)，包含插件源码、测试、截图和配套开发文档。

<Warning>
本文基于 Dockyard 插件 0.3.0。右键引用需要宿主支持 `assistant.composer.append_references`。复现前请确认部署版本包含该能力；不能只安装插件就假定未修改的 Xpert main 已支持。本文不将该接口描述为所有版本均可用的稳定能力。
</Warning>
...
如果要先运行示例，可在另一个目录克隆官方仓库，插件位于 `community/apps/dockyard`：
...
开发自己的插件时使用自己的功能分支，并核对所用源码与本文 0.3.0 示例的版本差异。记录插件、宿主和上游界面的 SHA，不要只记录“最新版”。本例使用 Node 22、插件 pnpm 8.15.8 和 SDK/contracts 3.18.4；宿主包管理器版本按宿主自己的 package.json 选择。
...
- [Dockyard 插件源码](https://github.com/xpert-ai/xpert-plugins/tree/main/community/apps/dockyard)
- [工作台远程组件](/zh-Hans/ai/plugin/plugin-sdk/remote-component)
- [插件发布和使用](/zh-Hans/ai/plugin/publish-and-use)
- [示例与六份开发材料](https://github.com/xpert-ai/xpert-plugins/tree/main/community/apps/dockyard/docs/interview)
